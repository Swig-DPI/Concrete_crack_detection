...
if __name__ == '__main__':
    ...
    predictions = model.predict_generator(self.validation_generator, steps = transfer_cnn.num_val/transfer_cnn.batch_size)
    pred_vals = predictions
    vec = np.vectorize(lambda x: 1 if x>0.6 else 0)
    predicted_class_indices=vec(predictions)
    ...
    vec = np.vectorize(lambda x: 1 if x>0.6 else 0)
    predicted_class_indices=vec(predictions)
    labels = (transfer_cnn.test_generator.class_indices)
    labels = dict((v,k) for k,v in labels.items())
    predictions = [labels[k] for k in predicted_class_indices.ravel()]
    filenames=transfer_cnn.test_generator.filenames[:len(predictions)]
    results=pd.DataFrame({"Filename":filenames,
                          "Predictions":predictions,
                          "Values":pred_vals.ravel()})
